...
<!-- 導覽列 -->
                <div class="header flex flex-col sm:flex-row gap-5  justify-between p-2 items-center">
                    <span class=" font-bold title">Mimir lab</span> 
  ...
  <!-- Your Flips按鈕 -->
                            <div v-if="flipsPopupVisible"
                                class="fixed left-0 top-0 w-[100vw] h-[100vh] bg-black/30 z-50 backdrop-blur-xl flex items-center justify-center">
                                <div class="bg-[#212c42] p-5 rounded-md relative m-3">
                                    <button class="absolute right-5 top-8 generic-hover" @click="closeFlipsPopup"
                                        id="close-button">
                                        <img class="w-4 h-4" src="@/icon/close.svg" alt="Close button">
                                    </button>
                                    <h1 class="text-3xl text-center mb-5"> Your Flips </h1>
                                    <div class="pb-5 flex items-center justify-center">
                                        <span
                                            class="py-2 px-5 bg-gray-500 rounded-md w-max mx-auto hover:brightness-90 transition duration-150 select-none">
                                            {{ rewardEntryData ? `Reward Amount: ${rewardEntryData.rewardAmount}`: 'Nothing to claim' }}
                                        </span>
                                    </div>
                                    <div class="rounded-md w-full flex flex-col gap-3 max-h-[50rem] overflow-y-auto">
                                        <div v-if="rewardAmount===0">
                                            <svg class="mx-auto my-5 w-24 h-24" xmlns="http://www.w3.org/2000/svg"
                                                width="32" height="32" viewBox="0 0 48 48" data-v-b3f24199="">
                                                <mask id="ipSFolderFailed0" data-v-b3f24199="">
                                                    <g fill="none" stroke-linejoin="round" stroke-width="4" data-v-b3f24199="">
                                                        <path fill="#fff" stroke="#fff"
                                                            d="M5 8a2 2 0 0 1 2-2h12l5 6h17a2 2 0 0 1 2 2v26a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V8Z"
                                                            data-v-b3f24199=""></path>
                                                        <path stroke="#000" stroke-linecap="round"
                                                            d="m19 22l10 10m0-10L19 32" data-v-b3f24199=""></path>
                                                    </g>
                                                </mask>
                                                <path fill="currentColor" d="M0 0h48v48H0z" mask="url(#ipSFolderFailed0)"
                                                    data-v-b3f24199=""></path>
                                            </svg>
                                            <p class="text-center px-3"> It seems like you don't have any recent flips,<br>
                                                try flipping some coins? </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<!-- Heads Tails按鈕 -->
            ...
            <div class="flex  flex-col transition-transform overflow-hidden  container lg:flex-row justify-center items-center mx-auto rounded-md font-ocr overflow-y-auto "
                style="position: relative;">
            <!-- <div class="flex  flex-col transition-transform overflow-hidden lg:flex-row container justify-center items-center mx-auto rounded-md font-ocr overflow-y-auto  bg-[#212c42cc] backdrop-blur-sm"
                style="position: relative;"> -->
                <div class="m-2 sm:p-12 flex flex-col items-center justify-center w-full lg:w-1/2 relative">
                    ...
                    <!-- <div id="bet-panel" class="m-2 w-full px-6 py-8 rounded-md bg-t-blue/75 relative"></div> -->
                    <div id="bet-panel" class="m-2 w-full px-6 py-8 rounded-md relative">
                        <div id="bet-contents" class="">
                            <div class="coin container" :class="{ heads: isHeads, tails: !isHeads, flipping: isFlipping }">
                                <img v-if="isHeads" src="@/images/heads.svg" alt="Heads" />
                                <img v-else src="@/images/tails.svg" alt="Tails" />
                            </div>
                            <div class="grid grid-cols-2 gap-2 text-white my-5 ">
                                <!-- <button :class="{ 'active-btn': selectedSide === 'heads' }" :style="{
                                    backgroundImage: selectedSide === 'heads' ? 'url(' + require('../ui/btn-active.svg') + ')' : 'url(' + require('@/ui/btn-inactive.svg') + ')',
                                    ...
                                    id="bet-btn-heads" @click="selectSide('heads')">
                                    HEADS
                                </button>
                                <button :class="{ 'active-btn': selectedSide === 'tails' }" :style="{
                                    backgroundImage: selectedSide === 'tails' ? 'url(' + require('../ui/btn-active.svg') + ')' : 'url(' + require('@/ui/btn-inactive.svg') + ')',
                                    backgroundSize: 'contain',
                                    backgroundRepeat: 'no-repeat',
                                    aspectRatio: '2.86047 / 1' 
                                }"
                                    class="text-2xl xl:text-4xl font-bold generic-hover flex flex-row items-center justify-center gap-4 text-white"
                                    id="bet-btn-tails" @click="selectSide('tails')">
                                    TAILS
                                </button> -->
                             <!-- HEADS 按鈕 -->
                                <button :class="{ 'active-btn': selectedSide === 'heads' }"
                                        @click="selectSide('heads')"
                                        class="button text-2xl xl:text-4xl font-bold generic-hover flex items-center justify-center gap-2 p-4 rounded-lg bg-white text-black border-2 border-black"
                                        id="bet-btn-heads">
                                    HEADS
                                </button>

                                <!-- TAILS 按鈕 -->
                                <button :class="{ 'active-btn': selectedSide === 'tails' }"
                                        @click="selectSide('tails')"
                                        class="button text-2xl xl:text-4xl font-bold generic-hover flex items-center justify-center gap-2 p-4 rounded-lg border-2 border-black"
                                        id="bet-btn-tails">
                                    TAILS
                                </button>
                            </div>
<!-- Betting Amount -->
                            ...
                            <div class="flex flex-row items-center my-3 bg-t-smoke rounded-md border-2 border-white">
                                <div class="relative rounded-l-md bg-gray-600 hover:opacity-75 p-2">
                                    ...
                                    <span class="after:content-['⌄'] after:absolute pr-4 after:right-1 after:bottom-3">SOL</span>
                                </div>
                                <input v-model="betAmount" type="text" id="bet"
                ...
                <div class="p-12 flex-1">  
                    ...
                    <div class="recent-plays rounded-md w-full flex flex-col gap-3 mb-10 font-ocr"  style="position: relative;">
                        <div 
                        v-for="(latestData, index) in recentPlays.slice(0, 6)" :key="index" class="flex flex-row justify-between items-center p-2 bg-t-blue/75 rounded-md">
                            <div>
                                <div class="flex flex-row justify-center gap-2 items-center">
                                    <img v-if="latestData.side === 1" :src="require('@/images/heads.svg')" alt="Heads" class="p-0.5" width="48"/>
                                    <img v-else-if="latestData.side === 2" :src="require('@/images/tails.svg')" alt="Tails" class="p-0.5" width="48"/>
                                    <p>
                                        <span>{{latestData.player.toBase58().substring(0, 5)}}</span> flipped {{ latestData.betAmount * 0.000001}} coin on
                                        {{ latestData.side === 1 ? 'Heads' : (latestData.side === 2 ? 'Tails' : play.coinSide) }} and
                                        <span 
                                        :class="{ 'font-bold text-green-500':  latestData.gameResult, 'font-bold text-red-500': !latestData.gameResult }">
                                        {{ latestData.gameResult ? 'Win!' : 'Lose!'   }} 
                                    </span>
                                </p>
                            </div>
                        </div>
...
<!-- Claim Rewards -->
        <div v-if="isModalOpen"
            class="fixed left-0 top-0 w-[100vw] h-[100vh] bg-black/30 z-50 backdrop-blur-xl flex items-center justify-center">
            <div class="bg-[#212c42] p-5 rounded-md relative m-3">
                <button class="absolute right-5 top-4 generic-hover" @click="closeModal" id="close-button">
                    <img class="w-4 h-4 " src="@/icon/close.svg" alt="Close button">
                </button>
                <h1 class="text-3xl text-center mb-3 p-5"> Claim Rewards</h1>
                <div class="pb-5 flex items-center justify-center">
                    <div class="rounded-md w-full flex flex-col gap-3 max-h-[50rem] overflow-y-auto">
                        <div v-if="rewardEntryData">
                            <p class=" font-bold mb-4 text-center ">Reward Amount: {{ rewardEntryData.rewardAmount }}</p>
                        </div>
                        <button
                        class="claim py-2 px-5 bg-gray-500 rounded-md mx-auto hover:brightness-90 transition duration-150 select-none mb-2"
...
//     //確認內容物是正確
//     // wallet.publicKey.value = wallet.publicKey.value ?? wallet.publicKey.value;
//     // wallet.signAllTransactions.value = wallet.signAllTransactions.value ?? wallet.signAllTransactions.value;
...
// }
    ...
    // const {rewardEntryData} = getRewardEntry();
    ...
    // //     try {
    // //         data = await program.account.rewardEntry.fetch(rewardEntryId);
    // //         // console.log('Already have reward entry:', rewardEntryData.value);
    // //         console.log('rewardAmount:', data.rewardAmount.toString());
    // //     } catch (error) {
    // //         console.log(error)
    // //     }
    // //     return {
...
.active-btn {
    /* background-color: rgb(226, 191, 64) !important; */
    background: linear-gradient(to right, rgba(255, 204, 0, 0.5), rgba(255, 222, 80, 0.5)) !important;
}

.active-btn:active,
.active-btn:focus {
  /* background-color: rgb(226, 191, 64) !important; */
  /* background: linear-gradient(to right, #ffcc00, #dda01b) !important; */
  background: linear-gradient(to right, rgba(255, 204, 0, 0.5), rgba(255, 222, 80, 0.5)) !important;
  /* border-color: rgb(214, 186, 104) */
}
...
.coin.flipping {
  animation: flip-animation 10s forwards;
}
...
.bg {
    background-image: url(@/images/4B9E871D0BC7F49D96A6CD30316612DF1F3F17A1.jpeg);
...
.title {
    ...
    font-size: 37px;
    /* font-weight: bold; */
    text-shadow: 2px 2px 4px rgba(207, 204, 204, 0.3);
...
.bg-blue {
    --tw-bg-opacity: 0.4;
    background-color:  #3d7eacc2;
    border-color: rgb(52, 120, 157)
...
:root {
    ...
    --swv-overlay-background-color: rgba(42, 13, 13, 0.8);
    --swv-modal-background-color: white;
    --swv-modal-text-color: #1f2937;
    --swv-modal-close-button-background-color: #f3f4f6 --swv-modal-close-button-text-color: #9ca3af;
